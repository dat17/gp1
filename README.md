...
# 後期15回目(2/9)
## ジャムの前に
- [Web上で簡単に3Dモデルにボーンを入れられるサイト](https://www.mixamo.com/)
- 追試の可能性が高い人への指示

## 2時間なんでもジャム
- 2時間で何かを完成させる
- 2時間が経過したら、ネットドライブに提出するか、可能ならGitHubなどに公開
- 簡単に発表する
- ネットドライブのフォルダーへ、以下をまとめて提出
  - 氏名のファイル名にしたテキストファイルに、作品のタイトルと作品概要を記載
  - ゲームのスクリーンショット1枚
...
## 提出場所
- `\\LANDISK-A601\disk\2017年\学生用フォルダー\GP_1\2-hour` に、半角英数で自分の名前のフォルダー(`tanaka`など)を作成して、その中に実行ファイル一式か、プロジェクトの中身をコピーしてください
